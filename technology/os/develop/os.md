...
为了解决实模式的缺陷，Intel 和 AMD 开发出了保护模式。在保护模式中，程序不能直接使用物理地址访问内存，只能通过线性虚拟地址访问内存。虚拟地址需要被转换为物理地址之后再去访问内存。地址转换是由处理器和 OS 共同协作完成的，处理器在硬件上提供地址转换部件，OS 提供转换过程中所需要的页表。

在保护模式下，段寄存器存储的不再是段基址，而是**全局描述符表**的选择子，类似于数组的索引，用于从全局描述符表获取内存的段描述信息。

基址寄存器不再只是 bx、bp，而是所有 32 位通用寄存器，变址寄存器也不再只是 si、di，而是除 esp 之外的所有 32 位通用寄存器。

#### 全局描述符表

全局描述符表的每一个表项称为段描述符，大小为 64 bit，用来描述各个内存段起始地址、大小、权限等信息。全局描述符表存储于内存中，由 GDTR 寄存器指向它。
